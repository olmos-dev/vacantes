...
@section('content')
    <h1 class="text-2xl text-center mt-10 capitalize">Editar Vacante: {{$vacante->titulo}}</h1>
    <form action="{{route('vacantes.update',['vacante' => $vacante->id]) }}" method="POST" class="max-w-lg mx-auto my-10">
        ...
        <div class="mb-5">
            ...
            <input id="titulo" type="text" class="p-3 bg-white rounded form-input outline-none w-full @error('titulo') is-invalid @enderror" name="titulo" value="{{ old('titulo') ?? $vacante->titulo }}" autofocus>
            @error('titulo')
        ...
        <div class="mb-5">
            ...
            <select name="skills[]" class="select block appearance-none border border-gray-200 text-gray-700 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500 p-3 bg-white w-full" multiple="multiple" data-placeholder="Seleccione al menos 3 opciones" style="width: 100%;">
                @foreach ($skills as $skill)
                    <option {{collect($habilidades)->contains($skill->id) ? 'selected' : '' }} value="{{$skill->id}}">{{$skill->nombre}}</option>                
                @endforeach   
            </select>
            @error('skills')
            <div class="bg-red-100 border border-red-500 text-red-700 px-4 py-3 rounded relative mt-3 mb-6" role="alert">
